...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if (typed,source) not in cache:
            ...
            if limit <= cache[(typed,source)][1] or cache[(typed,source)][0] <= cache[(typed,source)][1]: #如果result比limit小说明一定是准确值了
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    best_word = min(word_list,key=lambda s:diff_function(typed_word,s,limit))
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        add = minimum_mewtations(typed,source[1:],limit-1)+1 # 其实不用加source[0]，减少一次递归
        remove = minimum_mewtations(typed[1:],source,limit-1)+1 
        substitute = minimum_mewtations(typed[1:],source[1:],limit-1)+1 # source[0]不用加的
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[tpp[k][i]-tpp[k][i-1] for i in range(1,len(tpp[k]))] for k in range(len(tpp))]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for s in word_indices:
        index = min(player_indices,key=lambda num:get_time(times,num,s))
        fast_list[index] += [words[s]]
    return fast_list
    # END PROBLEM 10
